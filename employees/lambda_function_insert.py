...
def validate_transaction(transaction_data):
    ...
    for field, expected_type in expected_data_types.items():
        ...
        if field == 'name':
            # Check if 'name' is a string containing at least one space
            if not (isinstance(transaction_data[field], str) and re.match(name_pattern, transaction_data[field])):
                transaction_data["error"]="Missing surname or name"
                return False
        elif field == 'datetime':
            # Check if 'datetime' is a string in ISO format
            if not (isinstance(transaction_data[field], str) and is_valid_iso_datetime(transaction_data[field])):
                transaction_data["error"]="Problem with datetime ISO format"
                return False
        else:
            # Check if the field has the expected data type
...
def insert_transactions(cursor, transactions):
    ...
    values = [(record['id'], record['name'], record['datetime'], record['department_id'], record['job_id']) for record in transactions]
